...
if (hasInterface && !isDedicated) then {
    ...
    compileFinal "
...

            {
                if (isnil {_x getVariable ""IL_Taru_Action_Attach""}) then
                {
                    ...
                    IL_Taru_Action_Attach = _x addAction [""<img image='IgiLoad\images\load.paa' /><t color=""""#007f0e"""">Ankoppeln</t>"", ""[""""attach"""",_this] call IL_Taru_Do_Action;"", nil, 2, false, true, """",
                    ""[_this] call IL_Verify_Heli and {[vehicle _this] call IL_Verify_Pod} and {!([vehicle _this] call IL_Verify_Attached_Liga_Object)}""];
                };
                ...
                if (isnil {_x getVariable ""IL_Taru_Action_Deattach""}) then
                {
                    ...
                    IL_Taru_Action_Deattach = _x addAction [""<img image='scripts\IgiLoad\images\unload.paa' /><t color=""""#ff0000""""> Abkoppeln</t>"", ""[""""deattach"""",_this] call IL_Taru_Do_Action;"", nil, 2, false, true, """",
                    ""[_this] call IL_Verify_Heli and [_this] call IL_Verify_Liga_Altitude and {[vehicle _this] call IL_Verify_Attached_Liga_Object}""];
                };
                ...
                if (isnil {_x getVariable ""IL_Taru_Action_Drop""}) then
                {
                    ...
                    IL_Taru_Action_Drop = _x addAction [""<img image='scripts\IgiLoad\images\unload_para.paa' /><t color=""""#b200ff"""">!! Abwerfen !!</t>"", ""[""""drop"""",_this] call IL_Taru_Do_Action;"", nil, 2, false, true, """",
                    ""[_this] call IL_Verify_Heli and {[vehicle _this] call IL_Verify_Attached_Liga_Object}""];
                };
            } foreach [player];
    ...
    compileFinal "
        if (IL_Taru_Liga_DevMod) then {
            diag_log ""IL_Verify_Attached_Liga_Object called"";
        };
        _helico = (_this select 0);
        _object_Verifier = false;
        if (count (attachedObjects _helico) isEqualTo 0) exitwith {_object_Verifier};
        {
            if (_x isKindOf ""Land_Pod_Heli_Transport_04_Box_F"" || _x isKindOf ""Land_Pod_Heli_Transport_04_bench_F"" || _x isKindOf ""Land_Pod_Heli_Transport_04_covered_F"" || _x isKindOf ""Land_Pod_Heli_Transport_04_fuel_F"" || _x isKindOf ""Land_Pod_Heli_Transport_04_medevac_F"" || _x isKindOf ""Land_Pod_Heli_Transport_04_repair_F"" || _x isKindOf ""Land_Pod_Heli_Transport_04_ammo_F"" ) exitwith {_object_Verifier = true;};
        } foreach attachedObjects _helico;
        if (IL_Taru_Liga_DevMod) then {
             diag_log format[""IL_Verify_Attached_Liga_Object returns %1"",_object_Verifier];
        };
        _object_Verifier
    ";
    ...
    compileFinal "
        ...
        if (_pod isKindOf ""Land_Pod_Heli_Transport_04_Box_F"" || _pod isKindOf ""Land_Pod_Heli_Transport_04_bench_F"" || _pod isKindOf ""Land_Pod_Heli_Transport_04_covered_F"" || _pod isKindOf ""Land_Pod_Heli_Transport_04_fuel_F"" || _pod isKindOf ""Land_Pod_Heli_Transport_04_medevac_F"" || _pod isKindOf ""Land_Pod_Heli_Transport_04_repair_F"" || _pod isKindOf ""Land_Pod_Heli_Transport_04_ammo_F"") then {_pod_Verifier = true;};
        if (IL_Taru_Liga_DevMod) then {
	...
	compileFinal "
        if (IL_Taru_Liga_DevMod) then {
            diag_log ""IL_Verify_Liga_Altitude called"";
        };
        ...
        if (IL_Taru_Liga_DevMod) then {
             diag_log format[""IL_Verify_Liga_Altitude returns %1"",_allow_deattach];
        };
        _allow_deattach
    ";
   ...
   // IL_Taru_EH_Respawn = player addEventHandler ["Respawn", "[] spawn IL_Liga_Taru_Init;"];
};
    ...
    compileFinal "
        ...
        if (_helico isKindOf ""O_Heli_Transport_04_F"") then {_helico_Verifier = true;};
        if (IL_Taru_Liga_DevMod) then {
...
IL_Pod_Manager = 
	compileFinal "
    IL_Liga_CLient_Pod_Manager = _this;
    if (isDedicated || isServer) then
    {
        IL_Liga_CLient_Pod_Manager spawn IL_Server_Pod_Manager;
    }
    else
    {
        publicVariableServer ""IL_Liga_CLient_Pod_Manager"";
    };
";
"IL_Liga_CLient_Pod_Manager" addPublicVariableEventHandler IL_Pod_Manager;
...
compileFinal "
    ...
    if (hasInterface && !isDedicated) then
    {
        IL_Server_Client_Control spawn IL_Taru_Transmission; 
    }
    else
    {
        _heli = IL_Server_Client_Control select 1;
        _nearBy = (getPosATL _heli) nearEntities [[""LandVehicle"",""Ship"",""Air""], 50];
        {
            if (isPlayer _x) then {
...
if (isDedicated || isServer) then {
	...
	compileFinal "
        ...
        switch (_args select 0) do {
            case ""attach"": {
                 [_args select 1] spawn IL_Taru_Attach_Liga_Pod;
            };
            case ""deattach"": {
