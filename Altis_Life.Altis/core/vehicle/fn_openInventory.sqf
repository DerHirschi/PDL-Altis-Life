...
if (isNull _vehicle || !((isClass (missionConfigFile >> "LifeCfgVehicles" >> ( typeOf _vehicle ))) || _vehicle isKindOf "House_F")) exitWith {}; //Either a null or invalid vehicle type.
...
if(typeOf _vehicle in ["Land_ShelvesMetal_F"]) exitWith {
	...
	ctrlSetText[3501,format["Laden" + " - %1",getText(configFile >> "LifeCfgVehicles" >> (typeOf _vehicle) >> "displayName")]];
